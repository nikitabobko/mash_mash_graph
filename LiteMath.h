...
namespace LiteMath {
    ...
    static inline int4 make_int4(int a, int b, int c, int d) {
        int4 res;
        res.x = a;
        res.y = b;
        res.z = c;
    ...
    static inline int clamp(int u, int a, int b) {
        int r = (a > u) ? a : u;
        return (r < b) ? r : b;
    }
...
#define SQR(x) ((x)*(x))
    ...
    static inline float4 operator+(const float4 &u, const float4 &v) {
    ...
    static inline float4 &operator+=(float4 &u, const float4 &v) {
        u.x += v.x;
        u.y += v.y;
        u.z += v.z;
        u.w += v.w;
    ...
    static inline float4 &operator+=(float4 &u, float v) {
        u.x += v;
        u.y += v;
        u.z += v;
        u.w += v;
    ...
    static inline float4 catmullrom(const float4 &P0, const float4 &P1, const float4 &P2, const float4 &P3, float t) {
        ...
        return (P0 * (-tc + 2.0f * ts - t) + P1 * (3.0f * tc - 5.0f * ts + 2.0f) + P2 * (-3.0f * tc + 4.0f * ts + t) +
                P3 * (tc - ts)) * 0.5f;
    }

    static inline float4 lerp(const float4 &u, const float4 &v, float t) { return u + t * (v - u); }

    static inline float dot(const float4 &u, const float4 &v) {
    ...
    static inline float4 clamp(const float4 &u, float a, float b) {
        return make_float4(clamp(u.x, a, b), clamp(u.y, a, b), clamp(u.z, a, b), clamp(u.w, a, b));
    }
    ...
    static inline float3 operator+(const float3 &u, const float3 &v) {
    ...
    static inline float3 &operator+=(float3 &u, const float3 &v) {
        u.x += v.x;
        u.y += v.y;
        u.z += v.z;
        return u;
    ...
    static inline float3 &operator+=(float3 &u, float v) {
        u.x += v;
        u.y += v;
        u.z += v;
        return u;
    ...
    static inline float3 cross(const float3 &u, const float3 &v) {
    ...
    static inline float3 clamp(const float3 &u, float a, float b) {
        return make_float3(clamp(u.x, a, b), clamp(u.y, a, b), clamp(u.z, a, b));
    }
    ...
    static inline float2 clamp(const float2 &u, float a, float b) {
        return make_float2(clamp(u.x, a, b), clamp(u.y, a, b));
    }
    ...
    static inline float4x4 inverse4x4(float4x4 m1) {
        ...
        //
        float k = 1.0f / (m1.row[0].x * m.row[0].x + m1.row[0].y * m.row[1].x + m1.row[0].z * m.row[2].x +
                          m1.row[0].w * m.row[3].x);
    ...
    static inline void
    _my_frustumf3(float *matrix, float left, float right, float bottom, float top, float znear, float zfar) {
